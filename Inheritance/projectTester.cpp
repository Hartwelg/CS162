...
TEST_CASE( "RangedWeapon Basics" ) {
    ...
    RangedWeapon w2("Throwing knife", 1, 3, 5, 5);
    CHECK(w2.getDamage() == 1);
    CHECK(w2.getRange() == 3);
    CHECK(w2.getAmmo() == 5);
}
...
TEST_CASE( "Character takeDamage" ) {
    cout << "Running Character takeDamage..." << endl;

    Character c("Samus");

    //Each section will reset c
    SECTION("No armor") {
        c.takeDamage(5);
        CHECK(c.getHealthPoints() == 95);
    }
    ...
    SECTION("With armor") {
        ...
        c.aquire(&a);
        c.equip(0); //know that mail is 0

        //only take 1 damage as armor protects against 4
        c.takeDamage(5);
        CHECK(c.getHealthPoints() == 99);
    }

    SECTION("Heavy armor") {
        Armor a("Ironman suit", 100, 40000);

        c.aquire(&a);
        c.equip(0); //know that suit is 0

        //should take 0 damage
        c.takeDamage(5);
        CHECK(c.getHealthPoints() == 100);
    }
}

TEST_CASE( "Weapon useOn" ) {
    cout << "Running Weapon useOn..." << endl;

    Character c("Link");
    Character c2("Samus");

    //Each section resets c and c2
    SECTION("Basic weapon") {
        Weapon w("Mace", 4, 20);
        c.aquire(&w);
        c.equip(0); //know that the mace is in slot 0

        c.attack(c2, 3); //out of range - no damage
        CHECK(c2.getHealthPoints() == 100);

        c.attack(c2, 0); //in range, do damage
        CHECK(c2.getHealthPoints() == 96);
    }
    ...
    SECTION("Ranged weapon") {
        RangedWeapon w2("Bow", 3, 10, 20, 30);
        c.aquire(&w2);
        c.equip(0); //know that the bow is in slot 0

        c.attack(c2, 100); //out of range - no damage, use ammo
        CHECK(c2.getHealthPoints() == 100);
        CHECK(w2.getAmmo() == 19);

        c.attack(c2, 5); //in range, do damage, use ammo
        CHECK(c2.getHealthPoints() == 97);
        CHECK(w2.getAmmo() == 18);
    }


    SECTION("Ranged weapon out of ammo") {
        RangedWeapon w2("Bow", 3, 10, 0, 30);
        c.aquire(&w2);
        c.equip(0); //know that the bow is in slot 0

        c.attack(c2, 5); //no ammo, don't do anything
        CHECK(c2.getHealthPoints() == 100);
        CHECK(w2.getAmmo() == 0);
    }
